...
def render(source, preamble="", libs=["amssymb", "amsmath", "tikz", "circuitikz"], format="png"):
  document = f"""
...
\\end{{document}}
  ...
  with tempfile.TemporaryDirectory() as base:
    ...
    match format:
      case "png":
        subprocess.run(["pdftocairo", "-png", "-transp", "-r", "720", "-singlefile", f"{name}.pdf", name], cwd=path, capture_output=True, check=True)
